...
public class CartaoServiceTest {
	...
	public void should_have_money() {
		when(cartaoRepository.isSaldoSuficiente(anyInt(), anyInt(), Mockito.any(BigDecimal.class))).thenReturn(1);
		assertTrue(cartaoService.isSaldoSuficiente(123456, 12, new BigDecimal(200)));
	}
	
	@Test
	public void should_not_have_money() {
		when(cartaoRepository.isSaldoSuficiente(anyInt(), anyInt(), Mockito.any(BigDecimal.class))).thenReturn(0);
